...
from config.settings import LM_STUDIO_API, LM_STUDIO_BASE, AGENT_ROLES, SYSTEM_PROMPTS, ORCHESTRATOR_MODEL, DEFAULT_MODEL_ASSIGNMENTS, AGENT_DOS, AGENT_DONTS, LOG_DIR, LOG_LEVEL, LOG_FORMAT, LOG_DATE_FORMAT, LOG_MAX_BYTES, LOG_BACKUP_COUNT, EXPANDED_AGENT_ROLES
...
class LMStudioClient:
    ...
    async def load_model(self, model: str):
        """POST /api/v1/models/load — load a model into memory (LM Studio native REST)."""
        try:
            resp = await self.client.post(
                f"{self.base_url}/api/v1/models/load", json={"model": model}
            )
            return resp.json()
        except Exception as e:
            return {"error": str(e)}
    ...
    async def download_model(self, model: str):
        """POST /api/v1/models/download — download a model (LM Studio native REST)."""
        try:
            resp = await self.client.post(
                f"{self.base_url}/api/v1/models/download", json={"model": model}
            )
            return resp.json()
        except Exception as e:
            return {"error": str(e)}
    ...
    async def download_status(self, job_id: str):
        """GET /api/v1/models/download/status/:job_id — check download progress (native REST)."""
        try:
...
def register_engine_zones() -> None:
    """[A3] Populate the zone routing table.

    Registers every expanded (V2.2) role with its zone tag, then stamps the
    legacy mainline 9 as mainline in case they weren't touched by the expanded
    merge. This must run before any call to ``send_to_agent`` so that
    ``can_route`` has a complete view.

    Idempotent — safe to call multiple times.
    """
    register_zones_from_config(EXPANDED_AGENT_ROLES)
    register_mainline_legacy(list(AGENT_ROLES.keys()))
    print(
        f"[Engine] Zones registered: "
        f"{sum(1 for r in AGENT_ROLES if zone_for_role(r) == MAINLINE_ZONE)} mainline, "
        f"{sum(1 for r in AGENT_ROLES if zone_for_role(r) == HACKER_ZONE)} hacker-zone"
    )


# Fire zone registration at module import — engine is imported early in api
# startup, so this guarantees the routing boundary is active before the first
# request lands.
register_engine_zones()


async def init_agents():
    """Initialize all agents in the database (merged mainline + HZ roster)."""
    # Re-register zones in case AGENT_ROLES grew between imports.
    register_engine_zones()
    ...
    for role, info in AGENT_ROLES.items():
        ...
        agent_states[agent_id] = {
            ...
            "zone": zone_for_role(role),
        }
        # Defensive: if an expanded / HZ role isn't wired into AGENT_SKILLS /
        # MCP_AGENT_ROUTING / MEMORY_ACCESS yet, fall back to empty lists so
        # the whole init doesn't die (which would keep the lifespan from
        # finishing and leave port 8080 closed — = 502 wall in the UI).
        try:
            agent_states[agent_id]["skills"] = [s.name for s in get_skills_for_agent(role)]
        except Exception as e:
            agent_states[agent_id]["skills"] = []
            print(f"[Engine] warn: skills missing for {role}: {e}")
        try:
            agent_states[agent_id]["mcp_servers"] = [s.name for s in get_servers_for_agent(role)]
        except Exception as e:
            agent_states[agent_id]["mcp_servers"] = []
            print(f"[Engine] warn: mcp_servers missing for {role}: {e}")
        try:
            agent_states[agent_id]["memory_access"] = [m.value for m in get_agent_memory_access(role)]
        except Exception as e:
            agent_states[agent_id]["memory_access"] = []
            print(f"[Engine] warn: memory_access missing for {role}: {e}")
...
async def send_to_agent(
    ...
    *,
    ...
    [A2] Every routing call is gated by ``config.zones.can_route`` — a mainline
    agent cannot delegate to a hacker-zone agent (and vice versa) unless the
    session has ``bridge_open=True`` and both ends are bridge-capable
    (orchestrator ↔ hz_orchestrator only).

    Args:
        role:          The target agent role to dispatch to.
        task_description: Free-text task prompt.
        task_id:       Optional caller-supplied task id; generated if absent.
        source_role:   Who is doing the dispatching (keyword-only; defaults to
                       'orchestrator' for back-compat with existing callers).
        session:       Live SessionZoneState for the calling user. If None,
                       the call is treated as mainline (safer default).

    Raises:
        ZoneBoundaryError: if the requested routing would cross the mainline /
                           hacker-zone boundary without an open bridge.
    """
    # ----- [A2] Zone-boundary enforcement -------------------------------------
    active_zone = get_active_zone(session)
    if not can_route(source_role, role, active_zone, session):
        src_zone = zone_for_role(source_role)
        tgt_zone = zone_for_role(role)
        msg = (
            f"Zone boundary violation: {source_role!r} ({src_zone}) cannot "
            f"route to {role!r} ({tgt_zone}) while active_zone={active_zone!r}"
        )
        if session is not None and not session.bridge_open:
            msg += " (bridge is closed)"
        logging.getLogger("octopus.pipeline").warning(f"[Zones] {msg}")
        raise ZoneBoundaryError(msg)
    # --------------------------------------------------------------------------
...
{context_block}"""
    ...
    try:
        ...
        # Also save to Obsidian chat logs folder
        asyncio.create_task(save_chat_log_to_obsidian(
            task_id, role, task_description, content, provider_name, elapsed
        ))
    except Exception as e:
        logging.getLogger("octopus.memory").warning(f"[Memory] Failed to schedule memory save: {e}")
...
async def save_chat_log_to_obsidian(task_id: str, agent_role: str, user_message: str,
                                    assistant_response: str, provider: str = "", elapsed_ms: float = 0) -> None:
    """Save a complete chat log to Obsidian's chat_logs folder."""
    try:
        ...
        log_content = (
            ...
            f"---\n*This chat log was automatically saved by the Octopus V2 system.*\n"
        )
        path = f"chat_logs/{task_id}.md"
        save_result = await memory_client.save_note(path, log_content)
        logging.getLogger("octopus.memory").info(f"[Memory] Chat log save -> {save_result}")
    except Exception as e:
        logging.getLogger("octopus.memory").warning(f"[Memory] save_chat_log failed: {e}")


async def get_agent_info(role: str) -> dict:
    """Get comprehensive info about an agent including skills, MCP access, and memory."""
    ...
    mcp_servers = get_servers_for_agent(role)
    memory_access = get_agent_memory_access(role)

    return {
        "role": role,
        "status": state.get("status", "unknown"),
        "model": state.get("model", DEFAULT_MODEL_ASSIGNMENTS.get(role, "unassigned")),
        "skills": [{"name": s.name, "display_name": s.display_name, "category": s.category, "autonomous": s.autonomous} for s in skills],
        "mcp_servers": [{"name": s.name, "description": s.description, "category": s.category, "enabled": s.enabled} for s in mcp_servers],
        "memory_access": [m.value for m in memory_access],
        "dos": AGENT_DOS,
        "donts": AGENT_DONTS,
    }
...
async def get_memory_status() -> dict:
    ...
    return {
        "status": "connected" if healthy else "disconnected",
        "obsidian_url": config.obsidian_url,
        "vault": config.vault_name,
        "memory_types": [m.value for m in MemoryType],
        "operations": list(MEMORY_OPERATIONS.keys()),
    }
